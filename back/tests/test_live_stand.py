"""Сквозные проверки против живого стенда: поднятый ragu-api и индексы на диске.

По умолчанию пропускаются. Это не заглушка на сломанное — просто эти сценарии
проверяют развёртывание, а не код, и в голой копии репозитория проверять им
нечего. Запуск: `RAGU_LIVE_TESTS=1 pytest tests/test_live_stand.py` — из той же
сети, где отвечает сервис.
"""
...
pytestmark = pytest.mark.skipif(
    not os.getenv("RAGU_LIVE_TESTS"),
    reason="нужен поднятый ragu-api и индексы; RAGU_LIVE_TESTS=1 чтобы запустить",
)

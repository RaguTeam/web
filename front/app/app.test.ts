namespace $.$$ {
  ...
  $mol_test({
    "app.Body: $mol_scroll override is flex column"($) {
      const rule = style_rule(
        "$raggu_web_front_app",
        "raggu_web_front_app_body",
      );
    ...
    "app: every screen exists as sub-view"($) {
      ...
      $mol_assert_equal(
        v.Explorer() instanceof $raggu_web_front_explorer,
        true,
      );
    ...
    "explorer.communities: mock graph groups nodes by community"($) {
      ...
      $mol_assert_equal(
        colors.size,
        comms.length <= 15 ? comms.length : colors.size,
      );
    },
    ...
    "forcegraph: community filter dims outsiders, highlights internal edges only"(
      ...
      for (const e of v.graph_edges()) {
        const both_in =
          v.graph_nodes().find((n) => n.id === e.source)?.community === comm &&
